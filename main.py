...
template_dir = os.path.join(os.path.dirname(__file__), "templates")  # here are my templates files! 
jinja_env = jinja2.Environment(loader = jinja2.FileSystemLoader(template_dir), autoescape=True)  # load templates from template folder using jinja2
...
def validate_email(val):
    ...
    # Note: modified to allow a . before the @
...
def validate():
    ...
    # find if any first three fields are empty:
        # if so, display error message "This field cannot be empty" _____>next to field

    # check username and password:
        # if len(username) or len(password) < 3 or > 20:
            # if so, display error message "Must be between 3 and 20 characters"
        # if username or password contains a space:
            # if so, display error message "Spaces are not allowed"

    # check password and validation for identity:
        # if not same, print "Passwords must match"
    ...
    # here is where the data is being pulled from
    ...
    username_error = ""  # creates variable to hold error message for this field
    password_error = ""  # creates variable to hold error message for this field
    verify_error = ""  # creates variable to hold error message for this field
    email_error = ""  # creates variable to hold error message for this field
    ...
    # --===Check===-----
    if not username_error and not password_error and not verify_error and not email_error:  # error free passes to this:
